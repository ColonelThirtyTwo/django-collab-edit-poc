    ...
    return <div className="control-group">
        ...
        <div className="button-group">
            <Menu menuButton={<MenuButton>Heading...</MenuButton>}>
                {([1,2,3,4,5,6] as const).map(level => <FormatButton key={level} menuItem chain={c => c.toggleHeading({level})} active={e => e.isActive("heading", {level})}>
                    Heading {level}
                </FormatButton>)}
            </Menu>
...
function MyEditor(props: {
    ...
    const extensions = useMemo(() => EXTENSIONS.concat(
