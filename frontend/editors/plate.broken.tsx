...
const MyYjsAboveEditable: React.FC<{
  ...
  const { editor, useOption } =
    useEditorPlugin<MyYjsPluginConfig>(BaseMyYjsPlugin);
  ...
  if (!useOption("isConnected")) return null;
...
export function plateSingleLineEditor(
  el: HTMLElement,
  conn: Connection,
  key: string,
) {
...
export function plateAreaEditor(
  el: HTMLElement,
  conn: Connection,
  key: string,
) {}
