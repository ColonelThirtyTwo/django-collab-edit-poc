...
const MyEditor = (props: {connection: Connection, docKey: string}) => {
    ...
    const docObj = useMemo(() => props.connection.doc.get(props.docKey, Y.XmlText), [props.connection, props.docKey]);
...
export function slateSingleLineEditor(el: HTMLElement, conn: Connection, key: string) {
    const root = createRoot(el);
    root.render(<MyEditor connection={conn} docKey={key} />);
}


export function slateAreaEditor(el: HTMLElement, conn: Connection, key: string) {
    const root = createRoot(el);
    root.render(<MyEditor connection={conn} docKey={key} />);
}
